'use strict';
/**
 * seed-test-data-small.js
 * Smaller variant of seed-test-data.js for faster dev resets.
 *
 *   200 ingredients  (first 200 from the full template list, no variants)
 *   400 price quotes (2 per ingredient, different vendors)
 *
 * Everything else (units, price levels, 4 countries, brand partners, vendors,
 * categories, 48 recipes, 4 menus) is identical to the full seed.
 *
 * Can be run standalone:
 *   node api/scripts/seed-test-data-small.js
 *
 * Or imported by the /api/seed route (pass ?size=small or similar).
 */
...
// Total templates: 177. Cap at 200 (takes all 177 base + first 23 as "(Fresh)" variant).
...
function generateIngredientList() {
  ...
  for (const variant of VARIANTS) {
    for (const [name, category, unit, wastePct, basePrice] of ING_TEMPLATES) {
      ...
      if (result.length === 200) return result; // ← cap at 200
    }
  }
  return result;
}

// Recipe templates — identical to full seed
...
async function seedData(client, log = console.log) {
  ...
  ingList.forEach((ing, i) => {
    ...
    // Index base name without variant too (for recipe lookups)
    ...
    ingIdToUnit[ingIds[i]] = ing.unit;
  });

  // 8. Price Quotes (400 — 2 per ingredient, from different vendors)
  //    Quote 1: vendor at index  i % vendorIds.length
  //    Quote 2: vendor at index (i + 5) % vendorIds.length (offset by half = guaranteed different with 10 vendors)
  const quoteRows   = [];
  const quotedPairs = [];
  const half = Math.floor(vendorIds.length / 2); // = 5

  for (let i = 0; i < ingIds.length; i++) {
    const ingId = ingIds[i];
    const ing   = ingList[i];

    // Quote 1
    const v1Idx  = i % vendorIds.length;
    const price1 = +(ing.basePrice * (0.90 + (i % 5) * 0.05)).toFixed(4);
    quoteRows.push([ingId, vendorIds[v1Idx], price1, 1.0, ing.unit, true]);
    quotedPairs.push({ ingId, vendorId: vendorIds[v1Idx] });

    // Quote 2 — different vendor
    const v2Idx  = (i + half) % vendorIds.length;
    const price2 = +(ing.basePrice * (0.95 + (i % 3) * 0.05)).toFixed(4);
    quoteRows.push([ingId, vendorIds[v2Idx], price2, 1.0, ing.unit, true]);
    quotedPairs.push({ ingId, vendorId: vendorIds[v2Idx] });
  }
  ...
  // 9. Preferred vendors (first quote per ingredient wins per country)
  let pvCount = 0;
...
if (require.main === module) {
  (async () => {
    ...
    try {
      ...
      console.log('Seeding small test data (200 ingredients, 400 quotes)…');
