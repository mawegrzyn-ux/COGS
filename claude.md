...
*README last updated: April 2026 (session: Recipe duplicate + Unquoted-in-Recipes widget + sub-recipe yield unit display. New `POST /api/recipes/:id/duplicate` clones a recipe end-to-end in a single transaction — recipe row (name overridden, everything else preserved including translations), global items, and every market / PL / market+PL variation with its items (sort_order preserved). Audit log entry tagged `source: 'duplicate'` with `source_recipe_id`. Frontend Duplicate button next to Delete in the recipe header opens an app modal pre-filled with `"<original> (Copy)"`; on success the new recipe loads in the detail panel. New dashboard widget `recipe-unquoted-ingredients` with optional menu filter — backed by `GET /api/ingredients/unquoted-in-recipes?menu_id=<id>` which returns ingredients used in at least one recipe but with no active price quote (different from existing `missing-quotes` widget that lists *all* unquoted ingredients including orphaned catalog rows). Sub-recipe yield unit now displayed in the parent recipe's ingredients table — backend GET /recipes/:id queries (global + 3 variations) gained `COALESCE(sr.yield_unit_text, su.abbreviation) AS sub_recipe_yield_unit`; frontend falls back through prep_unit → base_unit_abbr → sub_recipe_yield_unit → "portion". Migration steps 147 (bugs) + 148 (backlog done) + 149 (changelog). DB: 87 tables, 146→149 steps, tools: 121, widgets: 21→22.)*
