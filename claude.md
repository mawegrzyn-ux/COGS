...
*README last updated: April 2026 (session: Jira 2-way sync Phase A+B + BACK-1942 Sales Items Excel view. Jira pull now syncs summary + description + labels + status + priority (was status + priority only), with last-write-wins conflict guard via `jira_remote_updated_at` vs local `updated_at`. New every-15-min cron `api/src/jobs/syncJira.js` calls shared `syncAll()` helper; outcome persisted to `mcogs_settings.data.jira_sync_status`. New `GET /api/jira/sync-status` endpoint + JiraSyncBanner component on Bugs & Backlog page (last-run metrics + Sync Now button). ADF-to-text helper flattens Jira descriptions. BACK-1942 shipped end-to-end and marked done: Sales Items page got a List ↔ Excel view toggle; Excel view is a 4-frozen-column spreadsheet grid (Name / Display / Type / Category) with one column per price level + one column per country, inline editing with optimistic auto-save + amber cell tint + ⟳ spinner while PUTs are in flight. Backend got `?include_prices=true` query param on `GET /api/sales-items` so the Excel view renders from one round-trip. Migration steps 140 (jira_remote_updated_at columns) + 141 (Jira sync changelog) + 142 (BACK-1942 changelog). DB: 87 tables (no change), 139→142 steps, tools: 121 (no change), widgets: 21 (no change).)*
