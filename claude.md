...
These are app-wide UX defaults — assume them on every new screen unless there's a documented reason not to. Reviewers should call out deviations.

**1. Drag-drop is the default for any list of orderable entries.**
- Categories, ingredients, recipes, modifier options, combo step options, sales items, menu items, kanban tiles, dashboard widgets, etc. — if it has a sort_order column, the UI must allow drag-drop reorder.
- Don't add up/down arrow buttons as the primary affordance. Drag-drop only; arrows are an accessibility fallback for keyboard users (also OK to add `↑ ↓` next to a drag handle for that, never as the only mechanism).
- Persist via a batch `/<entity>/reorder` endpoint that takes `[{id, sort_order}]` and writes inside a single transaction. Reuse the existing pattern in [recipes.js](api/src/routes/recipes.js) / [backlog.js](api/src/routes/backlog.js).
- Visual indicator: source row at 40% opacity, drop target gets an accent-coloured top border (or left border for kanban columns). Match the existing pattern in DashboardPage / BugsBacklogPage kanban.

**2. No native browser popups (`window.confirm` / `prompt` / `alert`).**
- Use the modal components from [ui.tsx](app/src/components/ui.tsx): `Modal`, `ConfirmDialog`, `DateConfirmDialog`, `Toast`. Build new modals as portal-rendered React components, never `confirm()`.
- Reasons: native popups can't be styled, block the event loop, can't be tested with Playwright, look out of place on a kiosk / PWA, and trip every browser's "this site is asking…" warning when fired in quick succession.
- For prompts that need an input, use `Modal` + `Field` with `autoFocus` on the input + Enter-to-submit.
- For destructive confirmations on shared/production data, prefer `DateConfirmDialog` (must type today's date as `ddmmyyyy`).
- Existing `window.confirm/prompt/alert` calls are tracked as a cleanup follow-up (BACK-* entries seeded with the rule); migrate them as you touch the surrounding code.
...
*README last updated: May 13 2026 (session: AWS Cognito dual-auth Phase 1 + Menu Builder polish wave. **Auth**: BACK-2364 Phase 1 dual-auth groundwork — new `mcogs_users.auth_provider` column (DEFAULT `'auth0'` + CHECK `auth0|cognito` + idx), middleware writes provider on create, `/me` + `/users` expose it, MeUser + AppUser TS types extended, Configuration → Users & Roles gains IdP column that hides itself when only Auth0 users exist. Zero behaviour change for current users (Cognito verifier path is a marked extension point — blocked on AWS provisioning). Full plan in [docs/COGNITO_INTEGRATION.md](./docs/COGNITO_INTEGRATION.md). BACK-2569 closed as duplicate of BACK-2364. **Menu Builder fixes re-applied to the Menu Builder tab** (originally interpreted as Sales Items > Combos catalog tab — user clarified they were about the inline combo rendering in MenuBuilderPage): BUG-1185 prominent 4-button type selector (recipe/ingredient/manual/sales_item — `sales_item` was missing entirely from this editor before this session; FullComboStepOption + SubComboOption types extended), BUG-1186 step cost summary now includes modifier adder via client-side avg(option.cost + sum(g.avg_cost × g.min_select)), BACK-2835 native HTML5 drag-drop on combo steps inside a menu row (POST /combos/:id/steps/reorder; comboId lazy-fetched + cached in a ref), BACK-2836 left accent bars + indent for hierarchy. **BUG-1191** combo option pickers showed "Recipe #27" instead of "Wings" — fixed by resolving display value from the loaded array. **Menu Builder polish**: BACK-2878 new + New Menu modal (name + market from useMarket + description, posts to /api/menus, auto-selects new menu), BACK-2879 fullscreen toggle (position fixed inset 0, Esc exits), BACK-2880 scroll position preserved on save (root cause: itemsLoading=true was unmounting ItemsList — spinner now only renders on initial load, refreshes show a thin accent progress bar at the top). **Configuration**: BACK-2881 new Modifier Cost × Number of Choices toggle in Configuration → COGS Calculation (multiplies modifier group display cost by min_select; default OFF; stored as `mcogs_settings.data.modifier_cost_uses_min_select`), BACK-2882 COGS Thresholds → COGS Calculation rename (sidebar tab + SettingsPage tab + Pepper tutorial + prose refs; internal route id kept stable). BACK-2837 Menu Entry header dedup + BACK-2883 Sidebar Menu Entry → Menu Builder rename (URL /menu-entry unchanged; i18n key menu_entry resolves to Menu Builder in en.ts; other 8 locales fall back). MenuBuilderPage gained a hideHeader prop so MenuEntryPage embeds it without duplicating the title. 49/49 changelog entries parse cleanly. Migration steps 172 → 175 (steps 172a/172b for Cognito, 173/173a for Combos tab fixes, 174/174a/174b for header rename + sidebar, 175u/175v for EOS retro flips, 175w/175x/175xs/175y/175z for changelog entries). DB tables unchanged at 88; one column ALTER (auth_provider). 7 retro tickets seeded: BACK-2878 to BACK-2883 + BUG-1191.)*

*README previous session: huge multi-feature drop covering menu builder, kiosk PWA, media library, and a major navigation refactor. **Menu builder**: BACK-2638 cost column + per-level COGS%, BACK-2639 manual joins the picker pattern, BACK-2640 + Add new shortcut per picker → BACK-2652 seamless same-tab Create-and-return flow with sticky ReturnToMenuBuilderBanner across /recipes /inventory /sales-items, BACK-2651 cost+COGS% folded into the price cell, BACK-2673 tax + cost laid out beside the price, BACK-2730 per-msi tax override (new mcogs_menu_sales_items.tax_rate_id col, PATCH /:id/tax, country-default fallback in country-price-levels GET, alt-bg dividers between level cells, TaxCell with portal dropdown), BACK-2770 drag-drop while grouped + cross-category move via /sales-items/bulk/category, BACK-2781 buckets follow Configuration → Categories sort_order + drag-drop category headers persist via /categories/reorder, BACK-2782 collapsible category buckets with collapse-all/expand-all toolbar, BACK-2793 **Menu Entry consolidation** — new MenuEntryPage with 4 tabs (Items/Combos/Modifiers/Menu Builder), all four tabs stay mounted across switches for state preservation, lazy initial mount, sidebar trades Sales Items + Menu Builder for one Menu Entry entry, **Menus → Menu Engineer** rename in sidebar + page header (URL stays /menus). **Kiosk**: BACK-2717 modifier images, BACK-2729 combo step option images (new mcogs_combo_step_options.image_url col), BACK-2728 **offline-resilient PWA** — kiosk-sw.js (precache shell + network-first menu data + cache-first images), kiosk-manifest.webmanifest (fullscreen portrait), new mcogs_kiosk_orders table with UNIQUE(order_uuid) for idempotent retries, IndexedDB queue (lib/kioskOfflineQueue.ts) + drainer (lib/kioskOrderDrainer.ts) firing on mount/online/30s/post-enqueue, top-right ConnectivityPill (offline / syncing / queue depth). **Media library**: BACK-2684 preserve original filename on upload, BACK-2695 drag-resizable list view columns, BACK-2706 TIFF support (sharp transcodes original to JPEG), BUG-1173 429 fix (skip rate-limit for /api/media/img + 1y immutable Cache-Control). **Bug fixes**: BUG-1164 migration JSONB parse error, BUG-1175 tax cell decimal display (formatPct helper), BUG-1180 combo cost ignored sales_item-typed step options (resolveOptionCost / _optionUnitCost gain a sales_item branch routing through linked si.recipe / ingredient / manual; loadComboData + loadComboStructure JOIN mcogs_sales_items + the linked recipe; recipe preload paths extended to include si.recipe_id), BUG-1181 shared-link grid view tiles now render sales item images (image_url added to /api/public/share/:slug/data response, aspect-square thumbnail at the top of each tile with initial-letter placeholder fallback). 19 backlog tickets shipped + 6 bugs resolved in one session. New helper script: api/scripts/_validate-changelog.js — JS-eval-aware JSON validator that runs against every mcogs_changelog INSERT to catch the BUG-1164 class of errors before commit; 38 changelog entries parse cleanly. DB: 87 → 88 tables, +2 column ALTERs (image_url + tax_rate_id). Migration steps span 170c through 170an + 171 + 171pre + 171pre0.)*
...
*README previous session: Modifier costs in COGS + standalone Pepper PWA + inline-edit focus fixes. Big multi-feature drop. New "Modifiers in COGS" toggle next to ⚡ What If (Menu Engineer + Shared View, defaults ON) — adds modifier cost into displayed cost/COGS using avg × min_select per attached modifier group. Backend exposes new shared helper `loadModifierCostAdders()` in cogs.js — full × min_select for SI-level groups (not in `cost_per_portion`), delta avg × (min_select − 1) for combo step option groups (since `calcComboCost` already adds avg×1). `modifier_cost_adder` field added per CogsItem in /cogs/menu-sales and per SharedItem in /public/share/:slug/data (market currency × qty). Cost cells show "59.84 (+₹34.81)" — recipe cost stays editable in the input, modifier portion read-only in brackets. Min-select=0 (optional) correctly resolves to 0; combo path delta cancels the implicit avg×1. Shared view applies adder + recomputes cogs_pct/gp_net per level on the fly via `effectiveItems` useMemo so all view modes update instantly. Modifier cost display in Menu Engineer expanded view: each combo/modifier option shows its cost in the Cost/ptn column; group/step headers show "avg ₹X.XX (₹min–₹max)". /menu-sales-items/:id/sub-prices enriched with per-option cost + per-group avg/min/max. Standalone Pepper PWA at https://cogs.macaroonie.com/pepper — full-viewport AiChat with isMobile=true forced for phones AND tablets, visualViewport keyboard inset, env(safe-area-inset-*) for iPhone notch. Dedicated /pepper-manifest.webmanifest (start_url + scope = /pepper, name "Pepper") swapped via PepperPage useEffect. PWA shortcut added to main manifest for "Ask Pepper". Auth0 switched to useRefreshTokens + cacheLocation:"localstorage" + offline_access scope (PWAs block third-party cookies the silent-iframe refresh depends on). Diagnostic [pepper] console.error in AiChat. CalcInput onCommit prop — fires once on Enter / blur after expression evaluation, separate from per-keystroke onChange. Inline-edit fixes: BUG-1027 Mix Manager scenario prices overlay, BUG-1028 What If disabled until data loads, BUG-1029 single-level qty fallback, BUG-1030 yield_qty onBlur commit pattern, BUG-1031 empty recipe Quote Coverage, BUG-1032 modifier-option recipes augmented in recipeItemsMap, BUG-1033 ingredient qty onCommit commit-only. Auth0 Refresh Token Rotation must be enabled on the SPA application in the dashboard for the standalone PWA fix to take effect. Migration steps 158-160. DB: 87 tables, 155→160 steps, tools: 122, routes unchanged.)*

*README previous session: Backlog Kanban + Suggest Priorities + Inventory grid sticky/default + + Quote stays in tab + group_name drop + widget click-to-add-quote + Pepper backlog tools + LanguageSwitcher fix. Big multi-feature drop. New Kanban view on the Bugs & Backlog page: 5 priority columns (Highest/High/Medium/Low/Lowest), drag tiles between columns to PUT priority, inline status dropdown on each tile (todo/in_progress/in_review/done/wont_do/backlog) with color-coded states, persisted view-mode toggle. New "✨ Suggest Priorities" button in kanban (dev-only) calls `POST /api/backlog/suggest-priorities` — Anthropic Haiku reads up to 200 open items and returns conservative priority adjustment proposals as `{summary, proposals: [{key, current, proposed, reasoning}]}`; frontend modal lets user accept/reject each before batch PUT. Inventory tabs default to grid view (persisted per browser); table headers sticky across DataGrid HeaderCell, ColumnHeader, and the standalone list-view tables. + Quote button on ingredient rows no longer switches tab — PriceQuotesTab is always-mounted via sr-only wrapper when triggered, modal portal renders over Ingredients. Dropped legacy `mcogs_categories.group_name` column (canonical is `group_id` FK since step 81). Two dashboard widgets (Missing Price Quotes + Unquoted in Recipes) gained click-through: tile click navigates to `/inventory?addQuote=<id>` which InventoryPage consumes via `useSearchParams` to auto-open the Add Quote modal pre-filled with the selected ingredient. Pepper backlog tools improved: `list_backlog` default 30→50, max 100→500, response now includes `{total, returned, truncated, rows}` + drops description from default SELECT to keep token use sane; new `get_backlog_stats` tool returns aggregate counts grouped by status / priority / item_type so Pepper can answer "how many?" in a single fast COUNT query without paginating. LanguageSwitcher dropdown now anchors top-down right-aligned (was bottom-up from sidebar footer) — fixes off-screen overflow now that the switcher lives in the top bar. Quick Links widget gained Grid/Column layout toggle in edit mode (column overrides per-tile width to col-span-12 for a single-column list). Migration steps 150-154 (BACK-1962/1001/1961 done + changelogs); step 155 = consolidated changelog for this session's user-flagged work. DB: 87 tables, 149→155 steps, tools: 121→122, widgets: 22.)*
