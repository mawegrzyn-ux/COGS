    ...
    <div className="p-6 max-w-4xl">
      ...
      <div className="space-y-3">
        {[
          ...
          { title: 'DataGrid — Inline-editable spreadsheet grid',  path: 'app/src/components/DataGrid.tsx',   detail: 'Generic editable grid supporting text / number / select / combo / derived cell types. Used in Compare Markets and Market Price Tool tabs.' },
    ...
    <div className="p-6 max-w-4xl">
      ...
      <div className="space-y-3">
        {[
          ...
          { title: 'Fix 4 — Currency conversion bug in Compare Markets',   symptom: 'Editing a price in a non-USD market saved the wrong value.',                                                                                   fix: 'Save-back used c.rate instead of dispRate (= c.rate / targetRate). Pass dispRate through to onSavePrice.',                                                                                                              file: 'app/src/pages/MenusPage.tsx' },
          { title: 'Fix 5 — ColumnHeader dropdown clipping',               symptom: 'Filter/sort dropdowns were cut off inside overflow-x: auto wrappers.',                                                                        fix: 'Changed to position: fixed with getBoundingClientRect() coordinates at z-index: 99999.',                                                                                                                                 file: 'app/src/components/ColumnHeader.tsx' },
          { title: 'Fix 6 — Pepper loses focus on every keystroke',        symptom: 'Typing in the chat textarea lost focus after each character. Focus not restored after AI response.',                                           fix: 'ChatPanel/HistoryPanel were defined inside AiChat(), creating new component identities on every render. Move both to module level. Add wasStreaming ref + useEffect to restore focus 100ms after streaming ends.',      file: 'app/src/components/AiChat.tsx' },
          { title: 'Fix 7 — Sidebar does not span full viewport height',   symptom: "The sidebar's green border stopped short of the bottom of the screen.",                                                                        fix: 'Change wrapper div from h-full to flex flex-col self-stretch so aside fills height definitively.',                                                                                                                       file: 'app/src/components/AppLayout.tsx' },
          { title: 'Fix 8 — Anthropic 400 error in multi-turn tool calls', symptom: 'messages.N.content.0.text.input_str: Extra inputs are not permitted — on message 9+ when Claude called multiple tools.',                     fix: 'agenticStream.js used input_str as a local streaming accumulator left on tool blocks. Destructure it off before pushing: const { input_str, ...cleanBlock } = currentBlock.',                                           file: 'api/src/helpers/agenticStream.js' },
        ].map(b => (
          <div key={b.title} className="border border-[#D8E6DD] rounded-lg p-3 bg-white">
            <p className="text-sm font-bold text-[#0F1F17]">{b.title}</p>
      ...
      <div className="space-y-2">
        {[
          ...
          { q: "Prices in Compare Markets show wrong currency or amount.",  a: "Check the market's exchange rate in Configuration → Location Structure. Use Configuration → Currency → Sync Rates to fetch live rates. Then reopen Compare Markets." },
